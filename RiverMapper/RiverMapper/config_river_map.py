...
class ConfigRiverMap():
    ...
    DEFAULT_river_threshold = (5, 600)  # used to be (16, 400) around STOFS-3D-Atl v6 (schism git f10c3eab)
    DEFAULT_min_arcs = 3
    ...
    def make_barrier_islands(cls):
        ...
        3. R_coef: the default value is 0.4;
              a larger value is needed here to make the barrier map smoother,
              because capturing the curvature of barrier islands is not as important
              as the capturing barrier height.
        4. length_width_ratio: the default value is 6.0;
              a larger value is used here to make the barrier map smoother.
        5. along_channel_reso_thres: the default value is (5, 300);
              a larger value is used here to nicely transition between
              the barrier and the ocean (which often have a resolution of 2000 m).
        '''
        return cls(
    ...
    def Levees(cls):
        ...
        to channels (in this case levees with a wider base and a narrower flat crown,
        i.e., 4 points/divisions along the cross-section)
